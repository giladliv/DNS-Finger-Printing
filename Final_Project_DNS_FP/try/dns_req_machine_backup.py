...
class DNS_FP_runner:
    ...
    def run_names_with_dns(self, is_recursive: bool = False, title: str = '',
                           label_session: str = '', progerss_bar=AliveProgBar):
        """
        ...
        @param progerss_bar: the progressbar class creation
        @return: (dict, str) - dict of the samples data, and time that taken in string
        """
        return self.__run_names_with_dns(self.DNS_address, self.list_names, is_recursive, title, label_session=label_session, progerss_bar=progerss_bar)

    def __run_names_with_dns(self, dns_main_ip, names, is_recursive: bool = False, title: str = '', label_session: str = '', progerss_bar=AliveProgBar):
        dict_names_ports = self.gen_port_names(names)

        self.dns_dict = {}
        time_sample = datetime.now()  # current date and time
        with progerss_bar(len(names), title=title) as bar:  # for showing progress of the packets
